...
BEGIN_MESSAGE_MAP(CMainFrame, CMDIFrameWndEx)
	...
	ON_WM_CREATE()
	ON_COMMAND_RANGE(ID_VIEW_APPLOOK_2007, ID_VIEW_APPLOOK_SCENIC, OnAppLook)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_APPLOOK_2007, ID_VIEW_APPLOOK_SCENIC, OnUpdateAppLook)
...
BOOL CMainFrame::CreateStatusBar()
{
	...
	apSBGroup1->AddButton(new CMFCRibbonButton(ID_FILE_PRINT_PREVIEW, _T(""), 0));
	apSBGroup1->AddButton(new CMFCRibbonButton(ID_FILE_SUMMARYINFO, _T(""), 1));
	...
	{
		apSBGroup2->SetImages(&images, NULL, NULL);
	}
	apSBGroup2->AddButton(new CMFCRibbonButton(ID_VIEW_GRID, _T(""), 0));
	m_wndStatusBar.AddExtendedElement(apSBGroup2.release(), _T("View Shortcuts"));
...
void CMainFrame::InitImageProcessingCategory()
{
	...
	pPanelWindow->Add(new CMFCRibbonButton(ID_FILTERING_WINDOWLEVEL, _T("Window Level\nn"), 0, 0)); // dlg
...
void CMainFrame::AdjustObjectSubmenu(CMFCPopupMenu* pMenuPopup)
{
	...
	if (pMenuPopup == NULL || pMenuPopup->GetParentPopupMenu() != NULL)
	{
		return;
	}
...
void CMainFrame::ShowOptions(int nPage)
{
	...
	apPageCustomize->AddCustomCategory(_T("Popular Commands"), lstPopular);

	// Add hidden commands:
	CList<UINT,UINT> lstHidden;
	m_wndRibbonBar.GetItemIDsList(lstHidden, TRUE);
	apPageCustomize->AddCustomCategory(_T("Commands not in the Ribbon"), lstHidden);

	// Add all commands:
	CList<UINT,UINT> lstAll;
	m_wndRibbonBar.GetItemIDsList(lstAll);
	apPageCustomize->AddCustomCategory(_T("All Commands"), lstAll);

	// Create property sheet:
	std::tr1::shared_ptr<CMFCPropertySheet> spPropSheet(new CMFCPropertySheet(_T("Options"), this, nPage));
	spPropSheet->m_psh.dwFlags |= PSH_NOAPPLYNOW;
	spPropSheet->SetLook(CMFCPropertySheet::PropSheetLook_List, 124 /* List width */);
