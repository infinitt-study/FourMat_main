...
CDrawObj::CDrawObj(const CRect& position,
	...
	const LOGPEN& logpen,
	BOOL bBrush,
	const LOGBRUSH& logbrush) : m_position(position), 
	m_pDocument(nullptr),
	m_bPen(bPen),
	m_logpen(logpen),
	m_bBrush(bBrush),
...
void CDrawObj::MoveTo(const CPoint& position)
{
	m_position.OffsetRect (position);
}
...
void CDrawPoly::MoveTo (const CPoint& position)
{
	...
	for (int i = 0; i < m_nPoints; i += 1)
	{
		m_points[i].x += position.x;
		m_points[i].y += position.y;
	}
}
